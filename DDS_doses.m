...
% time in seconds and drug dose in mg/s
   
convf=1/34000*(10^3); %milligrams-to-micromoles conversion factor
Drug_dose=drug_dose*convf; %convert to micromoles / day to use inside ODE function

InitialDose= initial_drug_dose*convf; % burst and initial amount in micromoles at t = 0; for t>0 Drug_dose vector stores drug release rate in micromoles/s
...
hold on
box on
plot(time(2:end)/(60*60*24), drug_dose(2:end), 'LineWidth', 2) % multiply by MW?
xlim([-2 200])
xlabel('Time (days)')
ylabel('DDS Drug release rate (mg/day)'); %% CHECK CONVERSION of units, is this moles or mg? 
...
%hold off
%timeweeks = (1:286); %total weeks (**CHANGE if time changes)
%timeweeks = t/7; %total weeks (**CHANGE if time changes)
...
pbaspect([1 1 1])
axis square
exportgraphics(figure(figure_count),sprintf('VEGFaq.png'), 'Resolution', 300)
figure_count = figure_count+1;
hold off
...
%Drug Concentration vitreous with different doses
figure(figure_count)
set(gca, 'FontSize', 14)
hold on
box on
for j = 1:length(doseo)
    plot(t, Drugv(j, :), 'LineWidth', 2)
end
%legend('Dose = 0.01mg', 'Dose = 0.05mg', 'Dose = 0.1mg', 'Dose = 0.5mg', 'Location','southeast');
xlabel('Time (days)');
ylabel('Abicipar Concentration (nM)');
xlim([-5 500])
title('Vitreous');
%fontsize(figure(9), 17, "points")
pbaspect([1 1 1])
axis square
exportgraphics(figure(figure_count),sprintf('Drugvitreous1.png'), 'Resolution', 300)
figure_count = figure_count+1;
hold off


save("DDS_doses.mat","drug_release_profile","drug_release_time","VEGFdoseaq","VEGFdosev","Drugaq","Drugc","Drugr","Drugs","Drugv","Data_time_at_target_vit_10","Data_time_at_target_vit_50", "Data_time_at_target_aq_10", "Data_time_at_target_aq_50", "in_vivo_suppression_time","in_vitro_suppression_time")
