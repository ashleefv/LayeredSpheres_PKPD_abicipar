...
for i = 1:length(thickness_scale) %for loop for each dose
...
hold on
box on
plot(time(1:end)/(60*60*24), drug_dose(1:end), 'LineWidth', 2)
xlim([-2 200])
xlabel('Time (days)')
...
title('Bi-Layered Varied \DeltaR, Constant R_{C}');
set(gca, 'FontSize', 12)
legend({'Upper IC_{50}', 'Lower IC_{50}'}, 'FontSize',14, 'Location', 'northwest');
pbaspect([1 1 1])
axis square
exportgraphics(figure(figure_count),sprintf('barbilayerPCL.png'), 'Resolution', 300)

save("bi_layered_PCL.mat","drug_release_profile","drug_release_time","VEGFdoseaq","VEGFdosev","Drugaq","Drugc","Drugr","Drugs","Drugv","Data_time_at_target_vit_10","Data_time_at_target_vit_50", "Data_time_at_target_aq_10", "Data_time_at_target_aq_50", "in_vivo_suppression_time","in_vitro_suppression_time")
